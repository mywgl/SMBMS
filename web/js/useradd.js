...
$(function(){
	...
	userCode.bind("blur",function(){
		...
		$.ajax({
			type:"GET",//请求类型
			url:path+"/jsp/user.do",//请求的url
			data:{method:"ucExist",userCode:userCode.val()==null? "":userCode.val()},//请求参数
			dataType:"json",//ajax接口（请求url）返回的数据类型
			success:function(data){//data：返回数据（json对象）
				if(data.userCode === "exist"){//账号已存在，错误提示
					validateTip(userCode.next(),{"color":"red"},imgNo+ " 该用户账号已存在",false);
				} else if (data.userCode === "No exist"){//账号可用，正确提示
					validateTip(userCode.next(),{"color":"green"},imgYes+" 该账号可以使用",true);
				}else{
					validateTip(userCode.next(),{"color":"red"},imgNo+ " 创建用户账号为空，请您重新输入!",false);
				}
			},
			error:function(data){//当访问时候，404，500 等非200的错误状态码
				if (data !=null)
